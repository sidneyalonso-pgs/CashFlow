...
function psFee(i: any) {
  return NO_REPASSE_MODELS.includes(i.modelo)
    ? Number(i.total)
    : Number(i.total_faturado) - Number(i.total_repasse);
}

function psRepasse(i: any) {
  return NO_REPASSE_MODELS.includes(i.modelo) ? 0 : Number(i.total_repasse);
}
...
export default async function FaturamentoDashboard({ searchParams }: { searchParams: { mes?: string } }) {
  const supabase = createClient();
  const mesAtual = new Date().toISOString().slice(0, 7);
  const mesFiltro = searchParams.mes ?? mesAtual;

  const [{ data: invoices }, { data: clients }] = await Promise.all([
    supabase
      .from("billing_invoices")
      .select("id, competencia, status, modelo, total, total_faturado, total_repasse, data_emissao, billing_clients(razao)")
  ...
  const all = invoices ?? [];
  const doMes = all.filter((i: any) => i.competencia === mesFiltro);
  const totalFee = doMes.reduce((s: number, i: any) => s + psFee(i), 0);
  const totalRepasse = doMes.reduce((s: number, i: any) => s + psRepasse(i), 0);
  const pendentes = doMes.filter((i: any) => i.status === "pendente").length;
  ...
  return (
    <div>
      ...
      {/* Filtro de mês */}
      <AutoSubmitForm className="flex gap-3 mb-4">
        <input
          type="month"
          name="mes"
        ...
        />
        {searchParams.mes && searchParams.mes !== mesAtual && (
          <Link href="/faturamento" className="text-sm text-ps-muted underline self-center">Mês atual</Link>
        )}
      </AutoSubmitForm>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard label={`Receita de fee (${mesFiltro})`} value={formatBRL(totalFee)} tone="green" />
        <StatCard label={`Repasse a pagar (${mesFiltro})`} value={formatBRL(totalRepasse)} tone="red" />
        <StatCard label="Faturas pendentes (mês)" value={String(pendentes)} tone="amber" />
          ...
          <tbody>
            {all.map((inv: any) => (
              <tr key={inv.id} className="border-t border-ps-navy/5 hover:bg-ps-bg-2/50">
                ...
                <td className="px-4 py-3 text-right tabular-nums text-red-600">{formatBRL(psRepasse(inv))}</td>
