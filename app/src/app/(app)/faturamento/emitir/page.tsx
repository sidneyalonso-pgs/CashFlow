...
export default async function EmitirFaturaPage() {
  ...
  const [{ data: clients }, { data: companies }, { data: subcontas }] = await Promise.all([
    ...
    supabase.from("billing_subcontas").select("client_id"),
  ]);

  const subcontaCounts: Record<string, number> = {};
  for (const s of subcontas ?? []) {
  ...
  return (
    <div>
      ...
      <EmitirFaturaForm clients={clients ?? []} companies={companies ?? []} subcontaCounts={subcontaCounts} />
    </div>
  );
}
