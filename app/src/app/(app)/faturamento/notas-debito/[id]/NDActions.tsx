...
import { baixarNotaDebito, cancelarNotaDebito, excluirNotaDebito } from "../../actions";
...
export function NDActions({ ndId, status }: { ndId: string; status: string }) {
  ...
  function handleExcluir() {
    if (!confirm("Excluir esta nota de débito definitivamente? Não dá pra desfazer.")) return;
    startTransition(async () => {
      const res = await excluirNotaDebito(ndId);
      if (res.error) { setError(res.error); return; }
      router.push("/faturamento/notas-debito");
    });
  }
  ...
  return (
    <div className="space-y-2 border-t border-ps-navy/5 pt-3">
      ...
      <button
        ...
        disabled={isPending}
        className="w-full bg-white border border-red-200 text-red-700 text-sm font-medium rounded-ps-sm py-1.5 hover:bg-red-50 disabled:opacity-60 transition-colors"
      >
        {isPending ? "..." : "🗑 Excluir"}
      </button>
    </div>
