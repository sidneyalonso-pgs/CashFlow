  ...
  return (
    <div className="bg-white p-10">
      ...
      <table className="w-full border-collapse text-xs">
        ...
        <tbody>
          {/* Todas as linhas sempre aparecem; sem movimento, qtd/valor/total ficam em branco (—) */}
          ...
          <tr className="border-b border-ps-navy/5"><td className="px-4 py-2.5 text-ps-ink">Bank Transfer</td><td className="px-4 py-2.5 text-ps-muted">BRL</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qbt > 0 ? qbt.toLocaleString("pt-BR") : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qbt > 0 ? N(valBankTransfer) : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums font-semibold">{qbt > 0 ? N(tBankTransfer) : "—"}</td></tr>
          <tr className="border-b border-ps-navy/5"><td className="px-4 py-2.5 text-ps-ink">PIX</td><td className="px-4 py-2.5 text-ps-muted">BRL</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qo > 0 ? qo.toLocaleString("pt-BR") : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qo > 0 ? N(valPixOut) : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums font-semibold">{qo > 0 ? N(tPixOut) : "—"}</td></tr>
          ...
          {/* Negadas: sem cobrança nesse contrato (ou qtd zerada) fica tudo em branco — nunca mostra o valor unitário sozinho */}
          <tr className="border-b border-ps-navy/5"><td className="px-4 py-2.5 text-ps-ink">Transações Negadas</td><td className="px-4 py-2.5 text-ps-muted">BRL</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qneg > 0 ? qneg.toLocaleString("pt-BR") : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qneg > 0 ? N(valNegadas) : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums font-semibold">{qneg > 0 ? N(tNegadas) : "—"}</td></tr>
          <tr className="border-b border-ps-navy/5"><td className="px-4 py-2.5 text-ps-ink">Refund/Estorno</td><td className="px-4 py-2.5 text-ps-muted">BRL</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qr > 0 ? qr.toLocaleString("pt-BR") : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{qr > 0 ? N(valRefund) : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums font-semibold">{qr > 0 ? N(tRefund) : "—"}</td></tr>
          <tr className="border-b border-ps-navy/5"><td className="px-4 py-2.5 text-ps-ink">Remessas Internacionais – Taxa de Câmbio</td><td className="px-4 py-2.5 text-ps-muted">BRL</td><td className="px-4 py-2.5 text-right text-ps-muted">—</td><td className="px-4 py-2.5 text-right tabular-nums text-ps-muted">{tRemessa > 0 && valRemessaPerc > 0 ? `${valRemessaPerc}%` : "—"}</td><td className="px-4 py-2.5 text-right tabular-nums font-semibold">{tRemessa > 0 ? N(tRemessa) : "—"}</td></tr>
        </tbody>
      </table>
      <div className="mt-5 flex justify-end">
        <div className="min-w-[300px] space-y-2">
